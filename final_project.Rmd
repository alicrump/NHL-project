...
# NA. Remove extra webscraper variables and change season variable so that 
# 2019-20 for example is now a variable called start which is 2019 and a 
# variable called end which is 2020. 
...
teams <- read_csv("raw-data/nhl_teams.csv", col_types = cols(
  ...
  mutate(color = case_when(team == "Anaheim Ducks" ~ "golden_rod3",
                           team == "Arizona Coyotes" ~ "tan",
                           team == "Boston Bruins" ~ "golden_rod3",
                           team == "Buffalo Sabres"~ "navy",
                           team == "Carolina Hurricanes" ~ "dodgerblue4",
                           team == "Columbus Blue Jackets" ~ "golden_rod3",
                           team == "Calgary Flames" ~ "brown2",
                           team == "Chicago Blackhawks" ~ "red",
                           team == "Colorado Avalanche" ~ "palevioletred4",
                           team == "Dallas Stars" ~ "seagreen4",
                           team == "Detroit Red Wings" ~ "red",
                           team == "Edmonton Oilers" ~ "orangered1",
                           team == "Florida Panthers" ~ "navy",
                           team == "Los Angeles Kings" ~ "snow3",
                           team == "Minnesota Wild" ~ "darkgreen",
                           team == "Montréal Canadiens" ~ "brown3",
                           team == "New Jersey Devils" ~ "red",
                           team == "Nashville Predators" ~ "golden_rod3",
                           team == "New York Islanders" ~ "dodgerblue3",
                           team == "New York Rangers" ~ "blue",
                           team == "Ottawa Senators" ~ "brown2",
                           team == "Philadelphia Flyers" ~ "chocolate1",
                           team == "Pittsburgh Penguins" ~ "black",
                           team == "San Jose Sharks" ~ "darkslategray4",
                           team == "St. Louis Blues" ~ "mediumblue",
                           team == "Tampa Bay Lightning" ~ "royalblue4",
                           team == "Toronto Maple Leafs" ~ "dodgerblue4",
                           team == "Vancouver Canucks" ~ "forestgreen",
                           team == "Vegas Golden Knights" ~ "lightgoldenrod3",
                           team == "Winnipeg Jets" ~ "maroon",
                           team == "Washington Capitals" ~ "firebrick3"))
...
# Join all the data frames into one and change season variable so that 
# 2019-20 for example is now a variable called start which is 2019 and a 
# variable called end which is 2020. 
...
```{r, move data}
# I'm moving my player and team data to the shiny app folder so that shiny
# can "see" it.
...
### Milestone 4
I created these graphics for Milestone 4 before I needed to change my season variable, so the x-axes are not quite right.  
...
teams %>% 
  filter(team == "Philadelphia Flyers", points != is.na(points), season != 2019) %>%
...
teams %>% 
  filter(team == "Philadelphia Flyers", points != is.na(points), season != 2019) %>%
  ggplot(aes(x = season, y = goals_for)) +
  geom_col(fill = "chocolate1") +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
